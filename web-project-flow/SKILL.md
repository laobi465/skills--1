...
【编码铁律（HARD）】
  ...
  13 严格遵循项目文档规范    触发：按项目文档开发 / 严格遵循规范 / 对标源码风格
     ...
     用途：8 条硬性规则 + 强制四步操作流程，写代码前必读项目文档和 /bdocs /bui 规范
...
2. **加载后行为**：按提示词内的指引开始执行（如 `/bstart` 加载后立即按完整版/精简版模板与用户对齐需求；`/baudit` 加载后立即询问项目路径；`/bstrict` 加载后立即询问用户项目文档路径并按"读文档 → 提问 → 读 /bdocs /bui 规范生成文档 → 写代码"四步流程执行）
3. **铁律叠加**：
